...
void KeyPressed(SDL_Keycode touche){
    ...
    switch (touche) {
        ...
        case SDLK_d:
		{
            //touche d appuyés
            //printf("D\n");
            if (gdpaddle<=WINDOW_WIDTH-taille_paddle && gdpaddle+taille_paddle < gdpaddle2){
				gdpaddle=gdpaddle+20;
			}
			if(gdpaddle>=WINDOW_WIDTH-taille_paddle ){
				gdpaddle=WINDOW_WIDTH-taille_paddle;
			}
            if(gdpaddle+taille_paddle >= gdpaddle2){
                gdpaddle=gdpaddle2-taille_paddle-10;
			}
            break;
        }
		case SDLK_h:
			...
			if(gdpaddle2<0)
				gdpaddle2=0;
			if(gdpaddle2 <= gdpaddle+taille_paddle){
        ...
        case SDLK_k:
		{
			...
			if(gdpaddle2>=WINDOW_WIDTH-taille_paddle ){
				gdpaddle2=WINDOW_WIDTH-taille_paddle;
			}
            break;
		}
        case SDLK_ESCAPE:
...
void drawPaddle()
{
	...
	drawRect(gdpaddle2, HAUTEUR_PADDLE, taille_paddle, 20);
}
...
void collisionPaddle()
{
	...
	if (ball_y>=HAUTEUR_PADDLE-TAILLE_BALL && ball_y<=HAUTEUR_PADDLE+30) //collision avec paddle haut bas
	{
        ...
        if (gdpaddle2<=ball_x && ball_x<(gdpaddle2+taille_paddle)){
            degre=(((ball_x-gdpaddle2)/taille_paddle)*120)+30;
			radian=(degre*M_PI)/180;
			speed_ball_y=-sin( radian )*speed_ball;
			speed_ball_x=-cos( radian )*speed_ball;
			speed_ball=speed_ball*1.07;
    ...
    if (ball_x==gdpaddle2-TAILLE_BALL || ball_x==gdpaddle2+taille_paddle+TAILLE_BALL)//collision avec paddle2 gauche droite
	{
		if (ball_y>=HAUTEUR_PADDLE && ball_y<=HAUTEUR_PADDLE+30)
		{
			speed_ball_x=-speed_ball_x;
